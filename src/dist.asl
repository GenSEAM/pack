...
(df emit-build-from-source-sh [(cfg DistConfig)] -> Str
  ...
  (str "#!/usr/bin/env bash\n"
       ...
       "set -eo pipefail\n\n"
       "echo \"🚀 [ASL] Building AgentScript from source (v" (.-version cfg) ")...\";\n"
       "ROOT_DIR=\"$(cd -P \"$(dirname \"${BASH_SOURCE[0]}\")/..\" && pwd)\"\n"
       "cd \"$ROOT_DIR\"\n\n"
       "# Check prerequisites\n"
       "command -v node >/dev/null 2>&1 || { echo \"✗ Node.js runtime required for memory daemon bootstrap.\"; exit 1; }\n"
       "command -v awk >/dev/null 2>&1 || { echo \"✗ AWK required for single-pass AST checking.\"; exit 1; }\n\n"
       ...
       "echo \"✓ Successfully built and symlinked ASL to $INSTALL_DIR/asl\";\n"
...
(df emit-npm-bin [(cfg DistConfig)] -> Str
  ...
  (str "#!/usr/bin/env node\n"
       ...
       "if (!targetBin) {\n"
       "  const daemonPath = path.join(rootDir, 'tools', 'asl-mem-daemon.mjs');\n"
